...
const ExperienceSection = () => {
    ...
    return (
        <section className="py-10 px-0 sm:px-5 flex flex-col items-center w-full">
            <h2 className="text-2xl sm:text-3xl font-bold mb-10 text-primary">Experience</h2>
            <div className="ml-6 space-y-10">
                {experiences.map((exp, index) => (
                    <div key={index} className="flex items-start justify-between gap-0 sm:gap-6">
                        <div className='relative flex'>
                            <div className='absolute w-1 h-70 sm:h-60 bg-border '></div>
                        ...
                        <div className="ml-0 sm:ml-15 bg-card text-foreground p-5 rounded-xl w-full shadow-md border border-border"> 
                            <div className="flex flex-col sm:flex-row gap-2 sm:gap-0 justify-between items-center mb-2">
                                <h3 className="text-sm sm:text-lg font-bold">
                                    ...
                                    <span className="text-primary">{exp.title.split(" ").slice(-1)}</span>
                                </h3>
                            ...
                            <p className="text-xs sm:text-sm text-justify text-muted-foreground mb-2">
                                <span className="text-primary font-semibold">
                                    <span className='text-foreground linier sm:hidden'>Institution </span>
                                    {exp.institution}
                                </span><br />
                                {exp.desc}
                            </p>
...
export default ExperienceSection; // Tidak perlu export default lagi jika sudah di atas